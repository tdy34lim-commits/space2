...
SEASON_COLORS = {"봄": "#2ca25f", "여름": "#e31a1c", "가을": "#E9E963", "겨울": "#3182bd"}
...
def build_incident_map(filtered: pd.DataFrame, fire_stations: pd.DataFrame | None = None) -> go.Figure:
    ...
    if fire_stations is not None:
        ...
        fig.add_trace(
            go.Scattermapbox(
                lon=fire_stations["lon"],
                lat=fire_stations["lat"],
                mode="text",
                text=["■"] * len(fire_stations),
                textfont={"size": 18, "color": "#000000"},
                customdata=fire_stations[["소방서명"]],
                hovertemplate="<b>%{customdata[0]}</b><br>소방서 반경: 2km<extra></extra>",
                showlegend=False,
            )
        )
        ...
        annotations=[
            {
                ...
                "text": (
                    ...
                    "<span style='color:#E9E963'>●</span> 가을&nbsp;&nbsp;"
